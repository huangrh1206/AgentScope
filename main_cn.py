...
class ThreeKingdomsWerewolfGame:
    ...
    async def setup_game(self, player_count: int = 9):
        ...
        # 写入角色分配表到日志
        if self.logger:
            self.logger.write_section("角色分配")
    ...
    async def run_game(self):
        ...
        try:
            ...
            for round_num in range(1, MAX_GAME_ROUND + 1):
                print(f"\n🌙 === 第{round_num}轮游戏开始 ===")
                if self.logger:
                    self.logger.write_section(f"第 {round_num} 轮", level=2)
                ...
                if self.logger and night_deaths:
                    self.logger.write_event(f"🌙 夜晚死亡：{'、'.join(night_deaths)}")
                ...
                if winner:
                    ...
                    if self.logger:
                        self.logger.write_section("🎉 游戏结束", level=2)
                        self.logger.write_event(winner)
                    return
                ...
                if self.logger:
                    self.logger.write_event(f"存活玩家：{format_player_list(self.alive_players)}")
                    self.logger.write_divider()
...
async def main():
    """主函数"""
    # 检查环境变量
    if "AIHubMix_API_KEY" not in os.environ:
    ...
    with logger:
        print("🎮 欢迎来到三国狼人杀！")
        print(f"📝 日志保存至：{logger.log_path}")
        
        # 创建并运行游戏
        game = ThreeKingdomsWerewolfGame(logger=logger)
        await game.run_game()
